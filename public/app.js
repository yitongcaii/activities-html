...
(function(){
  ...
  var _p=_d('eyJ0aXRsZSI6Iuivt+i+k+WFpeWIhuS6q+S4u+mimCIsInNwZWFrZXIiOiLkvIHlvq7kuK3oi7HmloflkI3vvIzlpoLvvJp6aGFuZ3NhbijlvKDkuIkpIiwic2hhcmVJbnRybyI6IueugOimgeaPj+i/sOWIhuS6q+WGheWuueS7t+WAvO+8jDN+NeS4quimgeeCueWNs+WPryJ9');
  function _fill(){
...
async function loadUserInfo() {
  let staffName = null, staffId = null;

  // 优先从专用接口取身份（OA 网关对 API 请求稳定注入身份头）
  try {
  ...
  // 兜底：部分网关只在页面响应注入身份头，再用 HEAD 读一次
  if (!staffName) {
    try {
      const h = await fetch(window.location.href, { method: 'HEAD' });
      staffName = h.headers.get('X-Staff-Name');
      staffId = h.headers.get('X-Staff-Id');
    } catch (e) {}
  }
  ...
  window.currentUser = currentUser; // 暴露到 window，供各 tab 脚本（如日历 isTopicOwner）读取
  ...
  await checkAdminAccess();
  if (typeof updatePointsBadge === 'function') updatePointsBadge();
  // 身份就绪后，若当前在日历页，重渲染以刷新「编辑我的分享」/「发起开课」按钮
  // （避免首屏渲染时 window.currentUser 尚未就绪导致按钮不显示）
  rerenderActiveTabForIdentity();
}

// 身份就绪后重渲染当前激活的日历页（修复首屏 race 导致的编辑按钮不显示）
function rerenderActiveTabForIdentity() {
  const cal = document.getElementById('tab-calendar');
  if (cal && cal.classList.contains('active') && typeof renderCalendar === 'function') {
    renderCalendar();
  }
}
...
function initTabs() {
  document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (tab === 'admin') { loadConfigs(); loadAdminBadges(); if (typeof renderMallAdmin === 'function') renderMallAdmin(); }
      if (tab === 'feedback') initFeedbackTab();
      if (tab === 'knowledge') initKnowledgeTab();
      if (tab === 'mall') { if (typeof showMallSection === 'function') showMallSection('shop'); if (typeof initMallOnboard === 'function') initMallOnboard(); }
    });
  });
}
