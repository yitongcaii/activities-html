  ...
  return `
    <div class="poster-topic-card">
      ...
      <div class="poster-speaker-intro">
        <span class="label">推荐面向人群：</span>${escapeHtml(topic.speakerIntro || '—')}
      </div>
      <div class="poster-topic-summary">
        <div class="label" style="display:block;margin-bottom:8px;">课程介绍：</div>
        <div style="padding-left:0;">${formatPosterIntro(topic.courseIntro || '—')}</div>
      </div>
      <div class="poster-topic-summary">
...
function renderPosterHtml(data) {
  const { dateLabel, bannerSrc, footerText, topics, themeId, category, hasBanner, headerTitle, location } = data;
  ...
  return `
    ...
    <div class="poster-inner poster-themed" id="posterInner" style="background:${theme.bodyBg || '#ffffff'};">
      ...
      <div class="poster-episode-bar" style="background:${theme.episodeGradient};">
        <span>${escapeHtml(dateLabel)}</span>${location ? `<span style="margin-left:12px;font-size:15px;font-weight:600;opacity:.95;">📍 ${escapeHtml(location)}</span>` : ''}
      </div>
      ...
      <div class="poster-signup-area">
        <span class="poster-btn-signup" style="padding:14px 56px;font-size:16px;background:linear-gradient(135deg, #f97316, #fb923c);">📢 点击报名+加入日程</span>
      </div>
    </div>`;
}
