...
function initDateOptions() {
  ...
  // 分享日期已改为原生日历输入（input[type=date]），无需再生成周三下拉选项
...
// ===== Form Submission =====
function initForm() {
  // 分享平台联动：切换平台时刷新分享时长建议标签
  const sp = document.getElementById('sharePlatform');
  if (sp) {
    sp.addEventListener('change', onSharePlatformChange);
  ...
  document.getElementById('topicForm').addEventListener('submit', async (e) => {
    ...
    const data = {
      ...
      sharePlatform: document.getElementById('sharePlatform') ? document.getElementById('sharePlatform').value : '',
      ...
      courseIntro: document.getElementById('courseIntro').value.trim(),
      shareIntro: document.getElementById('shareIntro').value.trim()
    };

    if (!data.title || !data.sharePlatform || !data.speaker || !data.shareDate || !data.speakerIntro || !data.courseIntro || !data.shareIntro) {
      showToast('请填写所有必填字段（含「分享平台」）', 'error');
      return;
    }
...
// 分享平台变更：联动分享时长建议
function onSharePlatformChange() {
  updateDurationTags();
}

// 分享时长随「分享平台」联动：渲染建议标签 + 写入隐藏 duration 值（总分钟数）
function updateDurationTags() {
  const sp = document.getElementById('sharePlatform');
  const box = document.getElementById('durationTags');
  const hidden = document.getElementById('duration');
  if (!sp || !box || !hidden) return;
  const v = sp.value;
  if (v === '博闻多识一堂课') {
    box.innerHTML = '<div style="display:flex;align-items:center;gap:10px;background:rgba(102,126,234,.10);border:1px solid rgba(102,126,234,.32);border-radius:10px;padding:8px 12px;">' +
      '<span style="font-weight:600;color:#3b51c4;white-space:nowrap;">博闻多识一堂课</span>' +
      '<span style="font-size:12px;color:#5b6b8c;">建议30分钟分享+10分钟互动</span></div>';
    hidden.value = '40';
  } else if (v === 'AISee实战沙龙') {
    box.innerHTML = '<div style="display:flex;align-items:center;gap:10px;background:rgba(46,160,110,.10);border:1px solid rgba(46,160,110,.32);border-radius:10px;padding:8px 12px;">' +
      '<span style="font-weight:600;color:#1f8a57;white-space:nowrap;">AISee实战沙龙</span>' +
      '<span style="font-size:12px;color:#5b6b8c;">建议20分钟分享+40分钟实操</span></div>';
    hidden.value = '60';
  } else if (v === '两个平台都可以') {
    box.innerHTML = '<div style="display:flex;align-items:center;gap:10px;background:rgba(102,126,234,.10);border:1px solid rgba(102,126,234,.32);border-radius:10px;padding:8px 12px;">' +
      '<span style="font-weight:600;color:#3b51c4;white-space:nowrap;">博闻多识一堂课</span>' +
      '<span style="font-size:12px;color:#5b6b8c;">建议30分钟分享+10分钟互动</span></div>' +
      '<div style="display:flex;align-items:center;gap:10px;background:rgba(46,160,110,.10);border:1px solid rgba(46,160,110,.32);border-radius:10px;padding:8px 12px;">' +
      '<span style="font-weight:600;color:#1f8a57;white-space:nowrap;">AISee实战沙龙</span>' +
      '<span style="font-size:12px;color:#5b6b8c;">建议20分钟分享+40分钟实操</span></div>';
    hidden.value = '40';
  } else {
    box.innerHTML = '<span style="font-size:12px;color:#9aa3b5;">请先选择上方「分享平台」，自动显示建议时长</span>';
    hidden.value = '0';
  }
}
...
function editMyTopic(topicId) {
  ...
  // 切到「提交分享主题」标签页，便于编辑（与日历嵌入态共用同一 SPA）
...
function initDraftAutoSave() {
  ...
  // 监听所有输入变化
  const fields = ['title', 'speaker', 'sharePlatform', 'speakerIntro', 'courseIntro', 'shareIntro', 'shareDate'];
...
function saveDraft() {
  ...
  const draft = {
    ...
    sharePlatform: document.getElementById('sharePlatform')?.value || '',
    speakerIntro: document.getElementById('speakerIntro')?.value || '',
    courseIntro: document.getElementById('courseIntro')?.value || '',
  ...
  // 只有有内容时才保存
  if (draft.title || draft.speaker || draft.speakerIntro || draft.courseIntro || draft.shareIntro) {
    try {
...
function restoreDraft() {
  ...
  try {
    ...
    if (!draft || (!draft.title && !draft.speaker && !draft.speakerIntro && !draft.courseIntro && !draft.shareIntro)) return;
    ...
    if (draft.sharePlatform) document.getElementById('sharePlatform').value = draft.sharePlatform;
    updateDurationTags();
    if (draft.speakerIntro) document.getElementById('speakerIntro').value = draft.speakerIntro;
    if (draft.courseIntro) document.getElementById('courseIntro').value = draft.courseIntro;
    if (draft.shareIntro) document.getElementById('shareIntro').value = draft.shareIntro;
    if (draft.shareDate) {
      document.getElementById('shareDate').value = draft.shareDate;
    }
