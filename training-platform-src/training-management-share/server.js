...
app.use((req, res, next) => {
  res.setHeader('Content-Security-Policy',
    ...
    "img-src 'self' blob: data: https://r.hrc.oa.com https://*.oa.com https://*.woa.com https://cdnjs.cloudflare.com; " +
    "font-src 'self' data: https://cdnjs.cloudflare.com https://fonts.gstatic.com; " +
    "style-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com https://fonts.googleapis.com; " +
    "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com https://res.wx.qq.com https://open.work.weixin.qq.com; " +
...
const memoryConfigs = new Map([
  ...
  ['speakerReminder', { enabled: false, template: '各位好，XX分享 下期分享将于下周三下午15:00-16:30进行，届时邀请 {speaker} 进行《{title}》的分享，大家别忘记安排好时间！\n\n[玫瑰]也麻烦 {speaker} 这边提前准备好分享材料：\n1、如有PPT，请提前上传至iWiki\n2、分享简介和海报会提前在群内宣传' }],
  ...
  ['rewardConfig', { enabled: false, rewardType: 'points', rewardAmount: 100, rewardDesc: '感谢分享，奖励已发放！' }],
...
app.post('/api/deposits', async (req, res) => {
  try {
    ...
    if (idx >= 0) {
      ...
      return res.json(memoryDeposits[idx]);
    }
    const deposit = { _id: 'dep_' + Date.now(), topicId, ...data, createdAt: new Date() };
...
async function initDefaultConfigs() {
  const defaults = [
    ...
    { key: 'speakerReminder', value: { enabled: false, template: '各位好，XX分享 下期分享将于下周三下午15:00-16:30进行，届时邀请 {speaker} 进行《{title}》的分享，大家别忘记安排好时间！\n\n[玫瑰]也麻烦 {speaker} 这边提前准备好分享材料：\n1、如有PPT，请提前上传至iWiki\n2、分享简介和海报会提前在群内宣传' } },
    ...
    { key: 'rewardConfig', value: { enabled: false, rewardType: 'points', rewardAmount: 100, rewardDesc: '感谢分享，奖励已发放！' } },
